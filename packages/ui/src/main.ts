...
const DECK: Record<Mode, Action[]> = {
  practice: [
    ...
    { label: 'Glow', title: 'Soft halo on everything that carries its own light — the POLLEN, the CELL frames, the zone paint. It is what makes a ball readable in the air against a dark hall. Costs a full-resolution pass plus five downsamples a frame, so it is the first thing to turn off on a slow laptop.', run: () => (scene.bloom = !scene.bloom), on: () => scene.bloom },
